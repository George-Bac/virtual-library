...
public class BookUtils {
   ...
   private BookUtils(Context context) {
      sharedPreferences = context.getSharedPreferences("shared_preferences_v1", Context.MODE_PRIVATE);
      SharedPreferences.Editor editor = sharedPreferences.edit();
      if (getAllBooks() == null) {
         editor.putString("all_books", new Gson().toJson(List.of(
      ...
      if (getCurrentlyReadingBooks() == null) convertFromListToJsonAndCommit(editor, "currently_reading_books");
      if (getAlreadyReadBooks() == null) convertFromListToJsonAndCommit(editor, "already_read_books");
      if (getWishListBooks() == null) convertFromListToJsonAndCommit(editor, "wish_list_books");
      if (getFavoriteBooks() == null) convertFromListToJsonAndCommit(editor, "favorite_books");
   }
   ...
   public List<Book> getAllBooks() {
      return convertFromJsonToList("all_books");
   }

   public List<Book> getCurrentlyReadingBooks() {
      return convertFromJsonToList("currently_reading_books");
   }

   public List<Book> getAlreadyReadBooks() {
      return convertFromJsonToList("already_read_books");
   }

   public List<Book> getWishListBooks() {
      return convertFromJsonToList("wish_list_books");
   }

   public List<Book> getFavoriteBooks() {
      return convertFromJsonToList("favorite_books");
   }
